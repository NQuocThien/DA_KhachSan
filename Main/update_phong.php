<?php
    ...
    $key=$_POST['chuoi'];
    $sql_selectethongtinphong = $pdo->prepare("SELECT * FROM `khachsan` ks , `phong` p WHERE ks.`MaKhachSan`=p.`MaKhachSan`And `MaChuoi`='$key'");
    ...
    // }
    // else{
    //     $sql_selectethongtinphong->execute();
    // }
            ...
            <form action="updatephong_submit.php" method="post" id="form_themphong">
                <div class="choose form_group">
                    ...
                    <select name="select_list-hotel" id="select_list-hotel" class="form-control">
                        ...
                        <?php 
                            while($row1 = $sql_selectkhachsan->fetch(PDO::FETCH_ASSOC))
                            {
